...
async function checkNewDatabase() {
  try {
    console.log('🔍 Vérification de la nouvelle base de données Railway...\n')

    // Vérifier la connectivité
    await prisma.$connect()
    console.log('✅ Connexion à la nouvelle base Railway réussie\n')

    // Compter les enregistrements
    const [brandsCount, categoriesCount, productsCount, variantsCount, imagesCount, usersCount] = await Promise.all([
      prisma.brands.count(),
    ...
    console.log('📊 Statistiques de la nouvelle base de données:')
    ...
    if (productsCount > 0) {
      ...
      console.log('⚠️  Aucun produit trouvé dans la nouvelle base de données')
      console.log('💡 La base de données est vide ou ne contient pas encore de données')
    }
