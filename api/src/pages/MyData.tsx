// web/src/pages/MyData.tsx
import { useState } from "react";

export default function MyData() {
    const [busy, setBusy] = useState(false);

    const hit = async (url: string, opts?: RequestInit) => {
        setBusy(true);
    ...
    return (
        <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
            ...
            <p>Exerce tes droits (accès, rectification, effacement, portabilité, limitation, opposition marketing).</p>
            ...
            <section>
                <h2>Rectification</h2>
                {/* à relier à un profil form existant ; exemple simplifié */}
                <form onSubmit={async e => {
                    e.preventDefault();
                    const form = new FormData(e.currentTarget as HTMLFormElement);
                    await hit("/api/rgpd/me", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
            ...
            <section>
                ...
                <button disabled={busy} onClick={() => hit("/api/rgpd/me/opt-out-marketing", { method: "POST" })}>Me désinscrire des communications marketing</button>
            </section>

            <section>
                <h2>Limitation du traitement</h2>
                <button disabled={busy} onClick={() => hit("/api/rgpd/me/restrict", { method: "POST" })}>Demander la limitation</button>
            </section>
                ...
                <button style={{ color: "white", background: "crimson" }}
                        ...
                        onClick={async () => {
                            if (confirm("Supprimer définitivement ton compte ?")) {
                                ...
                                window.location.href = "/"; // déconnexion & redir
                            }
